      ...
      switch(keyfromstring(tree->val)){
        case CODE_ASSIGN:
          if(is_expression(tree->right->val)){
            ...
            strcat(aux, generate_instruction("call", concat(tree->right->func_name, "_"), func_counter_string, NULL));
        ...
        default:
          switch(tree->node_kind){
            case FUNCTION_CHAR:
              aux = generate_instruction(concat(concat(tree->val, "_"), ":"), NULL, NULL, NULL);
